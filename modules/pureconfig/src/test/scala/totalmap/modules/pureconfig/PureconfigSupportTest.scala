...
class PureconfigSupportTest extends FunSuite {
  ...
  test("read and fail") {
    ...
    val mySet: NamedSet[String] = NamedSet("a", "b", "c")
    import mySet.allValues
    val result = pureconfig.loadConfig[TotalMap[mySet.Elem, String]](cfg)

    assert(
      result.left.get.head.description == "Cannot convert 'Map(b -> bar, a -> foo)' to TotalMap: following keys are missing: [c].")
  }
  ...
  test("read and succeed") {
    ...
    val mySet: NamedSet[String] = NamedSet("a", "b")
    import mySet.allValues
    val result = pureconfig.loadConfig[TotalMap[mySet.Elem, String]](cfg)
  ...
  test("read and succeed for boolean") {
    ...
    implicit def booleanMapConvert[T: ConfigConvert]: ConfigConvert[Map[Boolean, T]] =
      ConfigConvert[Map[String, T]].xmap[Map[Boolean, T]](
